...
class WorkflowTests(unittest.TestCase):
    ...
    def test_curriculum_contents_are_linked_and_objectives_are_free_text(self) -> None:
        state = create_session(self.course, agent=self.agent)
        ...
        self.assertIsInstance(state["curriculum_analysis"]["objectives"], str)
        self.assertTrue(state["curriculum_analysis"]["objectives"].strip())
        self.assertTrue(
            all(
                item["outcome_ids"]
    ...
    def test_manual_edit_creates_a_version_and_invalidates_only_on_save(self) -> None:
        ...
        edited["objectives"] = "Consolidar conhecimentos e promover a sua aplicação."
        ...
        self.assertEqual(
            updated["curriculum_analysis"]["objectives"],
            "Consolidar conhecimentos e promover a sua aplicação.",
        )
    ...
    def test_invalid_manual_edit_does_not_change_the_session(self) -> None:
        ...
        with self.assertRaisesRegex(AgentGenerationError, "objetivos gerais.*conteúdos"):
    ...
    def test_loading_a_legacy_session_adds_the_new_curricular_structure(self) -> None:
        ...
        self.assertIsInstance(restored["curriculum_analysis"]["objectives"], str)
    ...
    def test_schema_17_design_sequence_gains_the_teaching_activity(self) -> None:
        state = create_session(self.course, agent=self.agent)
        while state["current_stage"] != "pedagogical_design":
            state = review_current_stage(state, "approve", agent=self.agent)
        state["schema_version"] = 17
        for item in state["pedagogical_design"]["sequence"]:
            item.pop("teaching_activity", None)
        for version in state["versions"]["pedagogical_design"]:
            for item in version["sequence"]:
                item.pop("teaching_activity", None)
        ...
        self.assertTrue(
            all(
                item["teaching_activity"].strip()
                for item in restored["versions"]["pedagogical_design"][-1][
                    "sequence"
                ]
            )
        )
    ...
    def test_schema_18_separates_teaching_and_assessment_identifiers(self) -> None:
        ...
        for row in state["alignment_matrix"]:
            ...
            row["teaching_activity_ids"] = [
                teaching_map.get(identifier, identifier)
        ...
        for row in state["versions"]["alignment_matrix"][-1]:
            ...
            row["teaching_activity_ids"] = [
                version_teaching_map.get(identifier, identifier)
                for identifier in row["teaching_activity_ids"]
            ]
        state["revision_snapshots"] = [
            {
                "artifacts": {
                    ...
                    "alignment_matrix": deepcopy(state["alignment_matrix"]),
                }
            }
        ]
        ...
        for matrix in (
            restored["alignment_matrix"],
            restored["versions"]["alignment_matrix"][-1],
            restored["revision_snapshots"][0]["artifacts"]["alignment_matrix"],
        ):
            self.assertTrue(
                all(
    ...
    def test_legacy_session_at_the_old_second_stage_keeps_outcomes_current(self) -> None:
        ...
        self.assertIsInstance(regenerated["curriculum_analysis"]["objectives"], str)
        self.assertTrue(regenerated["curriculum_analysis"]["objectives"].strip())

    def test_schema_12_session_converts_objective_list_without_reopening_flow(self) -> None:
        state = create_session(self.course, agent=self.agent)
        while state["current_stage"] != "alignment_matrix":
            state = review_current_stage(state, "approve", agent=self.agent)
        legacy = deepcopy(state)
        ...
        for row in legacy["alignment_matrix"]:
            row["objective_ids"] = ["OG1"]
    ...
    def test_gpt_4o_mini_critic_omits_reasoning_parameter(self) -> None:
        ...
        with patch.dict(environ, {"OPENAI_API_KEY": "test-key"}, clear=False):
            result = critic.review(
                "curriculum_analysis",
                state,
                {"contents": [], "objectives": "Objetivos gerais."},
            )
    ...
    def test_schemas_follow_the_outcomes_first_sequence(self) -> None:
        ...
        self.assertEqual(curriculum_properties["objectives"]["type"], "string")
        ...
        sequence_properties = design_schema["properties"]["artifact"]["properties"][
            "sequence"
        ]["items"]["properties"]
